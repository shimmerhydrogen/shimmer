...
pm(:,1)=2/3*(Aplus'*p_k(:,k).^2+Aminus'*p_k(:,k).^2+(Aplus'*p_k(:,k)).*(Aminus'*p_k(:,k)))./((Aplus'*p_k(:,k))+(Aminus'*p_k(:,k)));
...
%     rho_n(:,1)=p_0(:,1)./cc2n;
      rho_n(:,1)=Den.*MM'; % [kg/m3] actual density of the gas (node based)
                           ...
                           % the previous line
      %*actual means that it is the density of the gas at the pressure and
      %the temperature at the specific node or pipe
      
      %recall of the EoS to calculate the nodal based quantities at
      %STANDARD CONDITION of T and p so to be able to calculate the
      %quantities listed few lines below
      iFlag=0;      
...
for ii=2:dimt
    ...
    ii %time step counter
     ...
     CC_gasM_k(:,:,k2)=CC_gasM(:,:,ii-1); %(è tra 0-100)
     CC_gas_k(:,:,k2)=CC_gas(:,:,ii-1);   %(è tra 0-1)
...
%     end
     ...
     CC_gasM(:,:,ii)= CC_gasM_k(:,:,k2); %(è tra 0-100)
     CC_gasM_ext(:,:,ii)= CC_gasM_ext_k(:,:,k2); %(è tra 0-100)
