function [D, ierr, herr, alpha_r] = DensityGERG(iFlag, T, P, x, dimn,gerg)
	...
	tolr = 1e-9;
	%   [gerg.Tcx,gerg.Dcx]=PseudoCriticalPointGERG(x,dimn,index); %Call PseudoCriticalPointGERG(x, Tcx, Dcx)

	if any(D >= 0)
		D(find(D>=0)) = P(find(D>=0))./RGERG./T(find(D>=0));               %Ideal gas estimate for vapor phase
		if iFlag == 2
			D(find(D>=0)) = gerg.Dcx(find(D>=0)) * 3;   %Initial estimate for liquid phase
		end
	else
	...
	for it = 1 : 50
		if any(vlog < -7) ||any(vlog > 100) || it == 20 || it == 30 || it == 40 || iFail == 1
			...
			nFail = nFail + 1;
			if nFail == 1
				D = gerg.Dcx * 3 ;   %If vapor phase search fails, look for root in liquid region
			elseif nFail == 2
				D = gerg.Dcx * 2.5;  %If liquid phase search fails, look for root between liquid and critical regions
			elseif nFail == 3
				D = gerg.Dcx * 2;    %If search fails, look for root in critical region
			end
		  vlog = -log(D);
		end
		D = exp(-vlog);
		[P2,Z,dPdDsave,alpha_r] = PressureGERG(T,D,x,dimn,gerg.Tr,gerg.Dr); %Call PressureGERG(T, D, x, P2, Z)
