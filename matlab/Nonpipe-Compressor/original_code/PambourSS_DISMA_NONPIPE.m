function [gasNet_Res, pos] = PambourSS_DISMA_NONPIPE(gasNet,Gsnam,P_set,Gguess,Pguess)
disp('helo')
time0=cputime;
     ...
     TN_M_k=(-Rf.*abs(G_k(:,k)).*G_k(:,k) -Ri.*G_n);
      ...
      if length(find(gasNet.Edges.COMP>0))==0
          ...
          for ccc=1:length(COMP)
            ...
            % beta=1.1;
            c1=-Ki*G_k(COMP(ccc),k)/pin*beta^ck; c2=Ki*G_k(COMP(ccc),k)/pout*beta^ck; c3=-Ki/ck*(beta^ck-1); d=gasNet.Edges.COMP_ctrl.PWR; %kW
                elseif gasNet.Edges.COMP_ctrl.RegType(ccc)==2
              %Pout
              c1=0; c2=1; c3=0; d=gasNet.Edges.COMP_ctrl.p_out;
                elseif gasNet.Edges.COMP_ctrl.RegType(ccc)==3
              %Pin
              c1=1; c2=0; c3=0; d=gasNet.Edges.COMP_ctrl.p_in;
              ...
              %Q
              c1=0; c2=0; c3=-1; d=gasNet.Edges.COMP_ctrl.Q;
                else
...
% for kkk=1:length(entry_p) %if pressure
%     aa=entry_p(kkk);
%       MATRIX_k(dimn+dimb+aa,aa)=1;
%       MATRIX_k(dimn+dimb+aa,dimn+dimb+aa)=0;
%       TN_L(entry_p(kkk))=P_set(kkk);
% end
% 
% for kkk=1:length(entry_l) %if flowrate entry (L)
%     aa=entry_l(kkk);
%       MATRIX_k(dimn+dimb+aa,dimn+dimb+aa)=1;
%       % MATRIX_k(dimn+dimb+aa,dimn+dimb+aa)=0;
%       TN_L(entry_l(kkk))=Gsnam(kkk);
% end
% 
% for kkk=1:length(exit) %if flowrate exit (L)
%     aa=exit(kkk);
%       MATRIX_k(dimn+dimb+aa,dimn+dimb+aa)=1;
%       % MATRIX_k(dimn+dimb+aa,dimn+dimb+aa)=0;
%       TN_L(exit(kkk))=Gsnam(kkk);
% end
...
% gasNet_Res.Nodes.MolComposition= MOLFRAC(:,:,end).*NaN; %p.u
    ...
    gasNet_Res=gasNet;
gasNet_Res.Edges.FLOWRATES= G_kf ; %kg/s
gasNet_Res.Nodes.PRESSURES=p_kf; %Pa
gasNet_Res.Nodes.G_EXE= L_k(:,k); %kg/s
