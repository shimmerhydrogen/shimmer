...
int load(sqlite3 *db, const optvector<int>& s_u2i, 
    ...
    using namespace pipe_QT;

    sqlite3_stmt *stmt = nullptr;

    std::string qlim = "SELECT pipe_quality_tracking_parameters.* "
	    "FROM pipe_quality_tracking_parameters INNER JOIN pipelines "
	    "ON pipe_quality_tracking_parameters.p_name = pipelines.p_name "
		"   AND pipe_quality_tracking_parameters.s_from = pipelines.s_from "
		"   AND pipe_quality_tracking_parameters.s_to = pipelines.s_to "
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        setting.nsegs = sqlite3_column_int(stmt, +params_col::nsegs);
