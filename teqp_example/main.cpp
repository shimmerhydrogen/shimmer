...
int main(int argc, char **argv) 
{
  ...
  const auto filter = shimmer_teqp_utilities::filter_components(x,
                                                                tolerance);

  const auto& names = teqp::GERG2008::component_names;
  const auto comps = shimmer_teqp_utilities::filter_collection(filter,
                                                               names);
  const auto mol_frac = shimmer_teqp_utilities::filter_collection(filter,
                                                                  x);
